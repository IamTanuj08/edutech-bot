...
GEN='''You create a standalone beginner lesson. Learner: 12th-grade graduate in India, basic English, zero AI background.
Use simple English, short sentences, examples, and explain every technical term when first used.
Use only the supplied context for factual claims. Never say RAG completely eliminates hallucinations.
Return strictly valid JSON only without markdown formatting. Format:
{
  "title": "...",
  "lesson": {
    "introduction": "...",
    ...
    "simple_example": {"problem": "...", "without_rag": "...", "with_rag": "..."},
...
EVAL='''You are a strict binary evaluator. You must return strictly valid JSON only.
PASS only if the criterion is clearly satisfied. No partial credit.
Check factual claims against supplied context.
Technical terms must be explained for a complete beginner.
RAG must not be described as eliminating hallucinations.
For each check in "checks", provide "name", "status" ("PASS" or "FAIL"), "reason", and "evidence".
Keep "evidence" concise (one plain sentence without nested quotes).

Return strictly valid JSON in this exact structure:
{
  "checks": [
    {"name": "<check_name>", "status": "PASS", "reason": "...", "evidence": "..."}
  ],
  "regeneration_instructions": []
}

Required checks: '''+', '.join(RUBRIC)

def client():
    if not GROQ_API_KEY: raise RuntimeError('GROQ_API_KEY missing. Copy .env.example to .env and add your Groq key.')
    return Groq(api_key=GROQ_API_KEY)

def _call_groq_json(system_prompt, user_prompt, temperature=0.2):
    for attempt in range(3):
        try:
            ...
            # Check if Groq failed validation but provided failed_generation text
            err_body = getattr(e, 'body', {})
            if isinstance(err_body, dict):
                failed_gen = err_body.get('error', {}).get('failed_generation')
                if failed_gen:
                    try:
                        # Auto-fix common LLM array delimiter glitch: }, "name" -> }, {"name"
                        repaired = re.sub(r'\}\s*,\s*\"([a-zA-Z0-9_]+)\"', r'},{"\1"', failed_gen)
...
def generate(topic,context,memory,feedback=None,demo=False):
    ...
    return _call_groq_json(GEN, prompt, temperature=0.2)

def evaluate(lesson,context):
    ctx='\n\n'.join(f"[{x['metadata'].get('title')}] {x['content']}" for x in context)
    prompt=f'REFERENCE:\n{ctx}\n\nLESSON:\n{json.dumps(lesson,ensure_ascii=False)}\nEvaluate now.'
    x = _call_groq_json(EVAL, prompt, temperature=0)
    by={c.get('name'):c for c in x.get('checks',[])}; checks=[]
